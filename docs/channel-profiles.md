...
- Facebook 根目录 `index.html + res.js` ZIP 交付；
- 下载报告中的渠道元数据和交付文件信息。

当前尚未完成：

- Google 的 `index.html + res.js` ZIP 输出；
...
| Facebook | ZIP：`index.html + res.js` | Facebook CTA | window load | ZIP 交付和 CTA 已接入，待 Meta Validator |
...
`index.html` 保留页面壳、渠道桥与 `res.js` 引用；`res.js` 承载 Payload、解码器和 Cocos 主运行时。CTA 优先调用 `FbPlayableAd.onCTAClick()`，本地不存在宿主 API 时回退商店地址。

详细结构和测试步骤见 [Facebook ZIP 交付](facebook-delivery.md)。

## 报告

Web 服务下载的 `game.report.json` 会附加渠道配置、预期交付格式、桥接方式、宿主全局对象和警告信息。

Liftoff 与 Facebook 已完成的交付报告都会包含：
...
- Facebook ZIP 根目录只有 `index.html` 与 `res.js`；
- ZIP CRC、解压与内容回环；
- 优化和未压缩任务不回归。
