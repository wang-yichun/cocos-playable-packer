...
- 先把通用渠道桥替换成只写入平台配置的轻量 bootstrap，不预先创建 `xsd_playable`；
- SDK 使用同步 `<script src="...">` 加载，顺序与样例一致；
- SDK 加载后捕获其新增或替换的 `xsd_playable.download/install`；
- 游戏 CTA 最终委托给 SDK 方法；
- 删除这两个渠道不使用的预览 MRAID 桩和通用商店跳转回退；
