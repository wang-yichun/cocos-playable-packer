...
/**
 * 读取 brotli-compress 的纯 JavaScript 解码器并转为普通脚本。
 *
 * 该解码器只在浏览器原生 DecompressionStream('brotli')
 * 不可用时才执行。最终生成的 HTML 不依赖网络和 node_modules。
 */
async function loadBrotliFallbackSource(): Promise<string> {
    let packageJsonPath: string;

    try {
        packageJsonPath = require.resolve(
            'brotli-compress/package.json',
        );
    } catch (_error) {
        throw new Error(
            '没有找到 brotli-compress。请先执行：'
            + 'npm i -D brotli-compress@1.3.3',
        );
    }
    ...
    const decoderModuleSource =
        await readFile(decoderPath, 'utf8');

    /*
     * brotli-compress/js.mjs 的结尾类似：
     *
     * export{aw as decompress};
     *
     * 将 ESM 导出替换为全局函数，方便普通 script 使用。
     */
    const exportPattern =
        /export\s*\{\s*([A-Za-z_$][\w$]*)\s+as\s+decompress\s*\}\s*;?\s*$/;

    const exportMatch =
        exportPattern.exec(decoderModuleSource);

    if (!exportMatch || !exportMatch[1]) {
        throw new Error(
            '无法识别 brotli-compress/js.mjs 的导出格式。'
            + '请确认安装的是 brotli-compress@1.3.3。',
        );
    }

    const localDecompressName =
        exportMatch[1];

    const browserSource =
        decoderModuleSource.replace(
            exportPattern,
            `window.__PACK_BROTLI_DECOMPRESS__=${localDecompressName};`,
        );
...
(function () {
    ...
    async function decompressBrotli(
        ...
        /*
         * 新版浏览器可能原生支持 Brotli。
         * 但许多 Chrome/Edge/WebView 版本只支持
         * gzip、deflate 和 deflate-raw，因此必须捕获
         * Unsupported compression format 并继续走回退。
         */
        if (
            ...
            try {
                ...
                console.log(
                    '[Playable Packer] 使用浏览器原生 Brotli 解码。'
                );
                ...
                console.warn(
                    '[Playable Packer] 浏览器原生 Brotli 不可用，'
                    + '切换到内嵌 JavaScript 解码器。',
                    error
                );
            }
        }
        ...
        var fallback =
            window.__PACK_BROTLI_DECOMPRESS__;
        ...
        var fallbackResult =
            fallback(compressedBytes);

        if (
            fallbackResult
            && typeof fallbackResult.then
                === 'function'
        ) {
            fallbackResult =
        ...
        console.log(
            '[Playable Packer] 使用内嵌 JavaScript Brotli 解码。'
        );
    ...
    function getBytes(input) {
        ...
        var start = entry.o;
        var end = start + entry.l;

        if (
            start < 0
            || end > archiveBytes.byteLength
        ) {
            throw new Error(
                'VFS 文件范围越界：'
                + key
        ...
        var bytes = archiveBytes.subarray(
            start,
            end
        );
...
async function main(): Promise<void> {
    ...
    for (const relativePath of filePaths) {
        ...
        archiveEntries[relativePath] = {
            ...
            o: totalRawBytes,
            l: outputBuffer.byteLength,
        };

        archiveParts.push(outputBuffer);
    ...
    console.log('');
    console.log(
        '正在执行 Solid Brotli Q11 压缩...'
    );

    const compressionStartedAt =
        performance.now();

    const archiveCompressedBuffer =
        brotliCompressSync(
            archiveRawBuffer,
            {
                params: {
    ...
    const compressionElapsedMs =
        performance.now()
        - compressionStartedAt;

    const packedArchive: PackedArchive = {
        v: 1,
        c: 'br',
        u: archiveRawBuffer.byteLength,
        b: archiveCompressedBuffer.toString(
            'base64',
        ),
    };
    ...
    const brotliFallbackSource =
        escapeJavaScriptSource(
            await loadBrotliFallbackSource(),
        );
    ...
    const html = [
        ...
        '<script>',
        brotliFallbackSource,
        '</script>',

        '<script>',
        `window.__PACK_FILES__=${filePayload};`,
        `window.__PACK_ARCHIVE__=${archivePayload};`,
    ...
    console.log('');
    console.log('Solid Brotli 单 HTML 已生成');
    ...
    console.log('');
    console.log(
        `打包文件数：${Object.keys(archiveEntries).length
        }`,
    );
    console.log(
    ...
    console.log(
        `Brotli 压缩大小：${formatBytes(
            archiveCompressedBuffer.byteLength,
        )}`,
    );
    console.log(
        `Brotli 压缩率：${(
    ...
    console.log(
        `压缩耗时：${compressionElapsedMs.toFixed(2)} ms`,
    );
    console.log(
