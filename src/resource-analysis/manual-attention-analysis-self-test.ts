...
const report = analyzeManualAttention(joint, optimization);
assert.equal(report.itemCount, 4);
assert.equal(report.highCount, 3);
assert.equal(report.mediumCount, 1);
assert.deepEqual(
  report.items.map((item) => item.category),
  ["large-scene", "oversized-image", "long-audio", "large-model"],
);
